...
@pytest.mark.asyncio
@respx.mock
async def test_translated_endpoint_shakespeare_happy_path(client: AsyncClient):
    ...
    respx.post(f"{fun_base}/{shakespeare_path}").mock(
        return_value=Response(
            200,
            json={"contents": {"translated": "Shakespeare Translated", "text": "x", "translation": "shakespeare"}},
        ),
    )
    ...
    assert resp.json()["pokemon"]["description"] == "Shakespeare Translated"


@pytest.mark.asyncio
@respx.mock
async def test_translated_endpoint_yoda_happy_path(client: AsyncClient):
    """Test yoda-happy-path."""
    # Arrange
    poke_base = str(settings.pokeapi_base_url).rstrip("/")
    fun_base = str(settings.funtranslations_base_url).rstrip("/")
    ...
    respx.post(f"{fun_base}/{yoda_path}").mock(
        return_value=Response(
            200,
            json={"contents": {"translated": "Yoda Translated", "text": "x", "translation": "yoda"}},
        ),
    )
    ...
    # Assert
    assert resp.status_code == 200
    assert resp.json()["pokemon"]["description"] == "Yoda Translated"
